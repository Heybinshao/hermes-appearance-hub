...
给 Hermes 桌面端用的**外观整合插件**：把主题、纸纹、字体、缩放、标签栏、密度、聊天背景、消息气泡、窗口透明、开场标识等外观设置收进一个状态栏入口，一键管理，设置持久化。

## 特性

- 无需构建、不改应用代码——单个 ESM 文件
- 状态栏「外观」入口，右键可显隐
- 主题（12 个，含 Binshao 暖纸）· 语言简/繁/EN · 霞鹜文楷 · 纸纹 · 缩放 · 标签栏 · 密度 · 聊天背景 · 消息气泡 · 窗口透明 · 开场标识
- 设置持久化，卸载清理注入、不留残留
...
点击状态栏「外观」按钮弹出，双栏铺开、高度减半，矮屏/大缩放不需要滚动。多数能力即时生效；标签栏、会话列表密度随下次布局变化生效：
...
2. 多数能力即时生效；标签栏、会话列表密度随下次布局变化生效：
   ...
   - **语言**：简/繁/EN 三键，绑定官方语言通道（位于顶部标题行、主题三档左侧）
   - **字体**：霞鹜文楷界面字体一键开关
   ...
   - **开场标识**：关/开即时生效（与官方设置页开关同键同步，官方页切换 hub 跟平）；开启时可选「原生文案」或「自定义」（输入字标与提示语，停手约半秒自动生效）；禁用插件会还原官方文案
